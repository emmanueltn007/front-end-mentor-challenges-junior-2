    ...
    <section className="flex flex-col gap-4 z-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {overviewTodayData.map(({ social, socialIcon, engagementType, likes, engagementNumber, engagementPercentage, likesNumber, likesPercentage, upIcon, downIcon }) => {
          return (
            <div className="grid grid-cols-2 gap-4" key={social}>
              <div className={`transition-all duration-300 ease-in-out ${isDarkMode ? "bg-[hsl(228,28%,20%)]" : "bg-[hsl(227,47%,96%)]"}`}>
                <div className="flex justify-between items-center">
                  <span>{engagementType}</span>
                  <img src={socialIcon} alt={`${social} icon`} />
                </div>
                <div className="flex justify-between items-center">
                  <span>{engagementNumber}</span>
                  <div className="flex items-center gap-2">
                    <img src={engagementPercentage < 0 ? downIcon : upIcon} alt="" />
                    {engagementPercentage < 0 ? engagementPercentage * - 1 : engagementPercentage}%
                  </div>
                </div>
              </div>

              <div className={`flex flex-col transition-all duration-300 ease-in-out ${isDarkMode ? "bg-[hsl(228,28%,20%)]" : "bg-[hsl(227,47%,96%)]"}`}>
                <div className="flex justify-between items-center">
                  <span>{likes}</span>
                  <img src={socialIcon} alt={`${social} icon`} />
                </div>
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    <img src={likesPercentage < 0 ? downIcon : upIcon} alt="" />
                    <span>
